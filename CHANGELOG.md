...
### Changed
- **BREAKING**: Automatic timeout detection is now **disabled by default**
  - `PeerConfig::timeout` changed from `Duration` to `Option<Duration>` (defaults to `None`)
  - `P2PConfig::peer_timeout` changed from `Duration` to `Option<Duration>` (defaults to `None`)
  - To enable timeout detection, explicitly set `timeout: Some(Duration::from_secs(30))` in config
  - This prevents unwanted disconnections in scenarios with long periods without data transfer
  - Applications that need disconnect detection can enable it explicitly
